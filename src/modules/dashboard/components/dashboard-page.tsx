  ...
  return <div className="page-stack dashboard-page">
    ...
    <section className="dashboard-grid dashboard-priority-grid">
      ...
      <article className="card card-secondary span-2 dashboard-wallet-card">
        <div className="card-head"><div><h2>Wallet aktif</h2><p className="section-helper">Ringkasan saldo wallet yang sedang digunakan.</p></div><Link href="/finance" className="text-link">Kelola</Link></div>
      ...
      <article className="card card-secondary span-2 dashboard-activity-card">
        <div className="card-head"><div><h2>Transaksi terbaru</h2><p className="section-helper">Aktivitas uang terbaru dari ledger.</p></div><Link href="/finance" className="text-link">Lihat semua</Link></div>
        {recent.length ? <div className="transaction-list dashboard-transaction-list">{recent.map((tx) => <div className="transaction-row dashboard-transaction-row" key={tx.id}><span className={`transaction-icon ${tx.type}`}><Icon name={tx.type === "income" ? "arrow-down" : tx.type === "expense" ? "arrow-up" : "swap"} size={16}/></span><span className="transaction-main"><strong>{tx.description}</strong><span className="dashboard-transaction-meta"><small>{walletNames.get(tx.walletId) ?? "Wallet"} · {formatDate(tx.date, { day: "2-digit", month: "short" })}</small></span></span><strong className={tx.type === "income" ? "money positive-text" : tx.type === "expense" ? "money negative-text" : "money neutral"}>{tx.type === "income" ? "+" : tx.type === "expense" ? "-" : ""}{formatCurrency(tx.amount)}</strong></div>)}</div> : <div className="empty-compact state-inline"><Icon name="swap" size={18}/><span>Belum ada transaksi. Tambahkan transaksi pertama dari tombol Tambah.</span></div>}
      </article>
