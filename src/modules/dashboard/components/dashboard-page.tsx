  ...
  return <div className="page-stack dashboard-page">
    ...
    <section className="dashboard-grid dashboard-priority-grid">
      ...
      <article className="card card-secondary span-2 dashboard-activity-card">
        ...
        {recent.length ? <div className="transaction-list dashboard-transaction-list">{recent.map((tx) => <div className="transaction-row dashboard-transaction-row" key={tx.id}><span className={`transaction-icon ${tx.type}`}><Icon name={tx.type === "income" ? "arrow-down" : tx.type === "expense" ? "arrow-up" : "swap"} size={16}/></span><span className="transaction-main"><strong>{tx.description}</strong><span className="dashboard-transaction-meta"><span className={`transaction-type-dot ${tx.type}`} aria-hidden="true"/><small>{tx.type === "income" ? "Pemasukan" : tx.type === "expense" ? "Pengeluaran" : "Transfer"} · {walletNames.get(tx.walletId) ?? "Wallet"} · {formatDate(tx.date, { day: "2-digit", month: "short" })}</small></span></span><strong className={tx.type === "income" ? "money positive-text" : tx.type === "expense" ? "money negative-text" : "money neutral"}>{tx.type === "income" ? "+" : tx.type === "expense" ? "-" : ""}{formatCurrency(tx.amount)}</strong></div>)}</div> : <div className="empty-compact state-inline"><Icon name="swap" size={18}/><span>Belum ada transaksi. Tambahkan transaksi pertama dari tombol Tambah.</span></div>}
      </article>
