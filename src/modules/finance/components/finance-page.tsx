...
export function FinancePage() {
  ...
  return <div className="page-stack">
    <section className="page-heading finance-heading">
      <div><span className="eyebrow">Finance</span><h1>Keuangan pribadi</h1><p>Saldo, arus kas, target, dan transaksi dengan ledger lokal sebagai sumber kebenaran.</p></div>
    ...
    <section className="analytics-grid">
      <article className="card card-secondary analytics-card">
        <div className="card-head analytics-head">
          <div><span className="card-label">Cashflow trend</span><h2>Arus kas</h2><p className="section-helper">Perbandingan pemasukan dan pengeluaran dari ledger asli.</p></div>
      ...
      <article className="card card-compact spending-card">
        <div className="card-head"><div><span className="card-label">Spending</span><h2>Kategori terbesar</h2></div></div>
    ...
    <section className="finance-grid">
      <article className="card card-secondary finance-wallets">
        <div className="card-head"><div><span className="card-label">Accounts</span><h2>Wallet</h2><p className="section-helper">Saldo dihitung dari saldo awal dan seluruh ledger.</p></div><button className="small-action" onClick={() => { setEditingWallet(null); setWalletOpen(true); }}><Icon name="plus" size={16}/>Tambah</button></div>
      ...
      <article className="card card-compact allocation-card">
        <div className="card-head"><div><span className="card-label">Allocation</span><h2>Komposisi wallet</h2><p className="section-helper">Porsi saldo positif pada setiap wallet aktif.</p></div></div>
    ...
    <section className="card card-secondary ledger-card">
      <div className="card-head transaction-head"><div><span className="card-label">Ledger</span><h2>Transaksi</h2><p className="section-helper">{transactions.length} hasil dari {state.transactions.length} transaksi</p></div>{activeFilterCount > 0 && <button type="button" className="filter-reset-inline" onClick={clearFilters}>{activeFilterCount} filter aktif · reset</button>}</div>
      ...
      {transactions.length ? <>
        <div className="table-wrap"><table className="data-table"><thead><tr><th>Aktivitas</th><th>Kategori</th><th>Wallet</th><th>Tanggal</th><th className="right">Nominal</th><th>Aksi</th></tr></thead><tbody>{transactions.map((tx) => { const wallet = walletById.get(tx.walletId); return <tr key={tx.id}><td><div className="transaction-cell"><span className={`transaction-icon ${tx.type}`}><Icon name={tx.type === "income" ? "arrow-down" : tx.type === "expense" ? "arrow-up" : "swap"} size={16}/></span><span><span className={`transaction-kicker ${tx.type}`}>{tx.type === "income" ? "Pemasukan" : tx.type === "expense" ? "Pengeluaran" : "Transfer"}</span><strong>{tx.description}</strong>{tx.note && <small>{tx.note}</small>}</span></div></td><td><span className="pill">{tx.category}</span></td><td>{wallet?.name ?? "-"}</td><td>{formatDate(tx.date, { day: "2-digit", month: "short", year: "numeric" })}</td><td className={`right money ${tx.type === "income" ? "positive-text" : tx.type === "expense" ? "negative-text" : "neutral"}`}>{tx.type === "income" ? "+" : tx.type === "expense" ? "-" : ""}{formatCurrency(tx.amount)}</td><td><div className="table-actions"><button type="button" onClick={() => setEditingTransaction(tx)}>Edit</button><button type="button" className="danger-link" onClick={() => removeTransaction(tx.id)}>Hapus</button></div></td></tr>; })}</tbody></table></div>
        <div className="mobile-transactions grouped-feed">{groupedTransactions.map((group) => <section className="transaction-day" key={group.key}><div className="transaction-day-label"><span>{dayLabel(group.key, now)}</span><small>{group.items.length} transaksi</small></div>{group.items.map((tx) => { const wallet = walletById.get(tx.walletId); return <div className="transaction-row transaction-row-actions" key={tx.id}><span className={`transaction-icon ${tx.type}`}><Icon name={tx.type === "income" ? "arrow-down" : tx.type === "expense" ? "arrow-up" : "swap"} size={17}/></span><span className="transaction-main"><span className={`transaction-kicker ${tx.type}`}>{tx.type === "income" ? "Pemasukan" : tx.type === "expense" ? "Pengeluaran" : "Transfer"}</span><strong>{tx.description}</strong><small>{tx.category} · {wallet?.name ?? "Wallet"}</small><span className="mobile-row-actions"><button type="button" onClick={() => setEditingTransaction(tx)}>Edit</button><button type="button" className="danger-link" onClick={() => removeTransaction(tx.id)}>Hapus</button></span></span><strong className={tx.type === "income" ? "money positive-text" : tx.type === "expense" ? "money negative-text" : "money neutral"}>{tx.type === "income" ? "+" : tx.type === "expense" ? "-" : ""}{formatCurrency(tx.amount, true)}</strong></div>; })}</section>)}</div>
